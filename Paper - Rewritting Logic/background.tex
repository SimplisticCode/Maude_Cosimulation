...
Complex co-simulation scenarios are subject to algebraic loops~\cite{Kubler2000} (\cref{fig:algebraic_example}) or step rejections~\cite{thrane2021} (\cref{fig:step_finding_scenario}).
...
\begin{compactenum}
  ...
  \item Given a \emph{parametric} or \emph{partially instrumented} scenario $\scenario$, where some SU parameters are unknown and where the instrumentation is incomplete i.e., not all input ports have been declared ether \emph{reactive} or \emph{delayed}, find concrete values for the the parameters, and a concrete instrumentation of the input ports, such that the resulting \emph{instrumented} scenario represent a system with desirable properties.
\end{compactenum}
