...
\begin{definition}[Abstract State]\label{def:runtime_state}
  The observable abstract state of SU $SU_c$\simon{Maybe different notation} is an element of the indexed set $\runstate{c} = \timebase \times \runstate{\inputs{c}} \times \runstate{\outputs{c}} \times \runstate{V_c}$, where:
  \begin{compactitem}
