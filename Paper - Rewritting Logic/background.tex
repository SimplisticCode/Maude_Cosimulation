...
\begin{definition}[Set Action]\label{def:setin}
  Setting a value $\tuple{t_{v}, x}$ on the input port $\inputvar{}$ of an SU using $\fset{}(\stateafter{}{t}, \inputvar{}, \tuple{t_{v}, x})$ transforms the abstract state of the SU according to:
  \begin{align*}
    \runstate{} 
  ...
  Where:
    ...
    \end{align*}
    The precondition says that the input must not have been assigned a new value since the SU was stepped.
    Furthermore, it requires that the value $\tuple{t_{v},x}$ respects the instrumentation of the input.
    The postcondition (defined below) ensures the value of the input $\inputvar{}$ is updated. 
    In addition, the timestamp of the input is updated.
    \begin{align*}
  ...
  \begin{definition}[Step Computation]\label{def:step}
    To step an SU using $\fdoStep{}(\stateafter{}{t}, H)$ changes the abstract state as:
    \begin{align*}
      \runstate{} 
...
The following section introduces a particular class of co-simulation scenarios subject to algebraic loops or step rejection.
